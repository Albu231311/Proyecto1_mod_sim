...
def algoritmo_genetico_tsp(distancias, coords, N, maxIter, pct_seleccion, pct_cruce, pct_mutacion):
    ...
    return mejor_ruta, mejor_dist, historial, tiempo_total

def guardar_resultados_csv(tipo_solucion, tiempo_ejecucion, solucion_encontrada, nombre_csv):
    """Guarda los resultados en un archivo CSV (agregando al existente)"""
    resultados = {
        'Archivo evaluado': [nombre_csv],
        'Tipo de Solución': [tipo_solucion],
        'Tiempo de ejecución (segundos)': [f"{tiempo_ejecucion:.4f}"],
        'Solución encontrada': [f"{solucion_encontrada:.2f}"]
    }
    
    df_nuevo = pd.DataFrame(resultados)
    nombre_archivo = 'resultados_GA.csv'
...
def mostrar_tabla_parametros(n_ciudades, tam_poblacion, num_iteraciones):
    """Muestra una tabla con los parámetros del problema"""
    # Calcular variables y restricciones del problema LP equivalente
    # Para TSP: n^2 variables binarias y aproximadamente 2n + n(n-1) restricciones
    num_variables = n_ciudades ** 2
    num_restricciones = 2 * n_ciudades + n_ciudades * (n_ciudades - 1)
    ...
    print("-"*80)
    print(f"{'Número de ciudades':<40} | {n_ciudades:>10}")
    print(f"{'Tamaño de la población del GA':<40} | {tam_poblacion:>10}")
    print(f"{'Número de iteraciones del GA':<40} | {num_iteraciones:>10}")
    print(f"{'Número de variables (problema LP)':<40} | {num_variables:>10}")
    print(f"{'Número de restricciones (problema LP)':<40} | {num_restricciones:>10}")
    print("="*80)
...
best, D, historial, tiempo_ejecucion = algoritmo_genetico_tsp(
    distancias, coords, N, maxIter, pct_seleccion, pct_cruce, pct_mutacion
)
...
print("Tiempo de ejecución: {:.4f} segundos".format(tiempo_ejecucion))
print("="*60)

# Mostrar tabla de parámetros
mostrar_tabla_parametros(len(coords), N, maxIter)

# Guardar resultados en CSV
guardar_resultados_csv("GA", tiempo_ejecucion, D, nombre_csv)